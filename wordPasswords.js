;(function(win, doc){
    ...
    win.PasswordGenerator = function() {
    ...
    PasswordGenerator.prototype = {
        ...
        makeSepArray: function (selectedSep) {

            var sSep = '';

            switch (selectedSep) {
                case '0':
                    sSep = this.sOther + ', ' + this.sNum;
                    break;
                case '1':
                    sSep = this.sNum;
                    break;
        ...
        doCalcs: function () {
            var selectedLength = this.length.options[this.length.selectedIndex].value,
                wordsCalc = Math.pow(this.aWords.length, selectedLength),
                sepCalc = Math.pow(this.makeSepArray(this.sep.options[this.sep.selectedIndex].value).length, selectedLength-1),
                totalCalc = (wordsCalc * sepCalc).toLocaleString(),
                statement = totalCalc + ' ' + this.lingo;

            this.possibilities.innerHTML = statement;
            this.clearResults();
        },

        makePassword: function (len) {

            var passWORD = '';

            var selectedSep = this.sep.options[this.sep.selectedIndex].value;

            this.aSep = this.makeSepArray(selectedSep);

            for (var x = 0; x < len; x += 1) {
                passWORD += '<span class="' + this.wordClass + '">' + this.aWords[this.getRandom(this.aWords.length)] + '</span>';
                passWORD += '<span class="' + this.sepClass + '">' + this.aSep[this.getRandom(this.aSep.length)] + '</span>';
            }

            passWORD = passWORD.slice(0, -26);
        ...
        makeItHappen: function () {
            ...
            var selectedCount = this.count.options[this.count.selectedIndex].value,
                selectedLength = this.length.options[this.length.selectedIndex].value;

            this.clearResults();

            for (var i = 0; i < selectedCount; i += 1) {

                this.results.innerHTML += this.makePassword(selectedLength) + '<br>';
